...
| 2026-03-28 | Phase C 生产补强：`server/main.py` 暴露 `/live-hls/*` 到 `LIVE_HLS_OUTPUT_DIR`，`.env.example` / `DEPLOY.md` / `services/media-egress/README.md` 补齐 `LIVE_HLS_BASE_URL`、`LIVE_HLS_OUTPUT_DIR`、`LIVE_EGRESS_WEBHOOK_SECRET` 的同机 origin 约定；为后续接入真实 egress 服务做生产路径预备 |
